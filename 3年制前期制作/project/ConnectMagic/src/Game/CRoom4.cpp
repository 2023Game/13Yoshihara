...
// 動く床の移動
const CVector MOVE_FLOOR_MOVE =			CVector( 0.0f,	 0.0f,	-120.0f);
// 動く床の移動時間
constexpr float MOVE_FLOOR_MOVE_TIME =	2.0f;

// 動く壁のオフセット座標
const CVector MOVE_WALL_OFFSET_POS =	CVector(-25.0f,	 0.0f,	-120.0f);
// 動く壁のスケール
const CVector MOVE_WALL_SCALE =			CVector( 3.0f,	 4.0f,	 1.0f);
// 動く壁の移動
const CVector MOVE_WALL_MOVE =			CVector( 50.0f,	 0.0f,	 0.0f);
// 動く壁の移動時間
constexpr float MOVE_WALL_MOVE_TIME =	2.5f;
...
void CRoom4::SetEnableRoom(bool enable)
{
	...
	if (enable) {
		...
		// 移動壁を追加
		saveMgr->AddSavableObj(mpMoveWall);
	}
	// 無効時
	else
	{
		...
		// 移動壁を削除
		saveMgr->DeleteSavableObj(mpMoveWall);
	}
}
